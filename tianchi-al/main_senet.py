...
import torchvision.models as models  # 子模块中包含以下模型结构： AlexNet VGG ResNet SqueezeNet DenseNet
import torchvision.transforms as transforms  # 对PIL.Image进行变换  对Tensor进行变换 Conversion Transforms
...
def main():
    ...
    train_data_list, val_data_list = train_test_split(all_data, test_size=val_ratio, random_state=21, shuffle=True,
    ...
    train_data = TrainDataset(train_data_list,
                              transform=transforms.Compose([
                                  ...
                                  FixedRotation([0, 30, 45, 60, 90, 135, 180, 270]),  # 添加了45、135 两个角度 ljc
    ...
    if evaluate:
        ...
        for epoch in range(start_epoch, total_epochs):
            ...
            monitor = pd.concat([monitor, pd.DataFrame({'train_acc': [train_acc], 'train_loss': [train_loss], 'val_acc': [precision], 'val_acc': [avg_loss]})])
